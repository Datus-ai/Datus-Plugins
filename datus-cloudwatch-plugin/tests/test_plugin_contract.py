...
def test_config_schema_lists_aws_and_plugin_fields():
    schema = CloudWatchPlugin.config_schema()
...
def test_validate_profile_shape_checks():
    ...
    assert CloudWatchPlugin.validate_profile({"secret_access_key": "${AWS_SECRET}"}) == []
    # every declared config_schema field is an accepted key
    assert CloudWatchPlugin.validate_profile(
        {f["name"]: "${X}" for f in CloudWatchPlugin.config_schema()}
    ) == []
    # unknown keys are reported
    errs = CloudWatchPlugin.validate_profile({"bogus_key": 1})
    assert errs and "bogus_key" in errs[0]
    # a malformed endpoint URL is caught, a placeholder is not
    assert CloudWatchPlugin.validate_profile({"endpoint_url": "ftp://x"})
    assert CloudWatchPlugin.validate_profile({"endpoint_url": "${EP}"}) == []
