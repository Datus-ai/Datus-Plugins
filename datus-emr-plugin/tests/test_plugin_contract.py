"""The Datus plugin contract for datus-emr-plugin."""
...
from datus_emr_plugin.plugin import EmrPlugin

PKG_DIR = Path(__file__).resolve().parents[1] / "datus_emr_plugin"
GROUPS = ("clusters", "steps")


def test_constructor_accepts_profile_keyword():
    plugin = EmrPlugin(profile={"name": "prod", "region": "us-east-1"})
    assert plugin.profile["name"] == "prod"
    assert EmrPlugin().profile == {}


def test_run_cli_help_returns_zero(capsys):
    rc = EmrPlugin(profile={}).run_cli(["--help"])
    assert rc == 0
...
def test_run_cli_unknown_command_returns_usage_error():
    assert EmrPlugin(profile={}).run_cli(["frobnicate"]) == 2


def test_run_cli_unknown_profile_key_is_config_error(capsys):
    rc = EmrPlugin(profile={"bogus_key": 1}).run_cli(["clusters", "list"])
    assert rc == 3
    assert "bogus_key" in capsys.readouterr().err


def test_skills_dir_is_class_reachable_and_exists():
    skills = Path(EmrPlugin.skills_dir())
    assert (skills / "emr" / "SKILL.md").is_file()
    assert (skills / "emr-setup" / "SKILL.md").is_file()


def test_system_prompt_unconfigured_points_to_setup_skill():
    text = EmrPlugin.system_prompt({})
    assert "not configured" in text
    assert "emr-setup" in text
...
def test_config_schema_lists_aws_and_plugin_fields():
    ...
    assert {"cluster_id", "log_uri"} <= names  # plugin extras


def test_validate_profile_shape_checks():
    # ${ENV_VAR} placeholders are opaque and never rejected
    assert EmrPlugin.validate_profile({"secret_access_key": "${AWS_SECRET}"}) == []
    # every declared config_schema field is an accepted key
    assert EmrPlugin.validate_profile(
        {f["name"]: "${X}" for f in EmrPlugin.config_schema()}
    ) == []
    # unknown keys are reported
    errs = EmrPlugin.validate_profile({"bogus_key": 1})
    assert errs and "bogus_key" in errs[0]
    # a malformed endpoint URL is caught, a placeholder is not
    assert EmrPlugin.validate_profile({"endpoint_url": "ftp://x"})
    assert EmrPlugin.validate_profile({"endpoint_url": "${EP}"}) == []
...
def _cli_command_paths() -> list:
    from datus_emr_plugin.cli import build_parser
...
def test_cli_permissions_shape():
    perms = EmrPlugin.cli_permissions()
...
def test_cli_permissions_patterns_reference_real_commands():
    ...
    for rules in EmrPlugin.cli_permissions().values():
        for patterns in rules.values():
            for pattern in patterns:
...
def test_cli_permissions_dangerous_never_auto_run():
    ...
    for profile, rules in perms.items():
        ...
        assert not any(_matches("steps add", h) for h in allow_heads), f"steps add must never auto-run ({profile})"
...
def test_cli_permissions_routine_promoted():
    ...
    for command in ("clusters list", "steps logs"):
        assert allowed("normal", command) and allowed("auto", command)
    assert not allowed("normal", "steps cancel")
    assert allowed("auto", "steps cancel")
...
def test_package_never_imports_datus():
    result = subprocess.run(
        [
            sys.executable,
            "-c",
            "import sys; import datus_emr_plugin.plugin; import datus_emr_plugin.cli; "
