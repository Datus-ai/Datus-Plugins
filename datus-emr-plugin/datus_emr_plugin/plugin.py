...
class EmrPlugin:
    ...
    def config_schema(cls) -> List[Dict[str, Any]]:
        """Profile fields for the `/plugins` TUI form (shared AWS keys + EMR extras)."""
        return aws_config_schema(extra_fields=[
            {"name": "cluster_id", "description": "Default EMR cluster ID for `steps` commands"},
            {"name": "log_uri", "description": "S3 base URI for step logs (for `steps logs`)"},
        ])

    @classmethod
    def validate_profile(cls, profile: Dict[str, Any]) -> List[str]:
        """Shape-check a candidate profile before it is saved (${VAR} left opaque)."""
        return validate_aws_profile(profile, extra_keys=("cluster_id", "log_uri"))
