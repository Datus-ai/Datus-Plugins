"""The Datus plugin contract for datus-s3-plugin."""
...
from datus_s3_plugin.plugin import S3Plugin

PKG_DIR = Path(__file__).resolve().parents[1] / "datus_s3_plugin"
GROUPS = ("ls", "stat", "cat", "cp", "sync", "mv", "rm", "select", "buckets")


def test_constructor_accepts_profile_keyword():
    plugin = S3Plugin(profile={"name": "prod", "region": "us-east-1"})
    assert plugin.profile["name"] == "prod"
    assert S3Plugin().profile == {}


def test_run_cli_help_returns_zero(capsys):
    rc = S3Plugin(profile={}).run_cli(["--help"])
    assert rc == 0
...
def test_run_cli_unknown_command_returns_usage_error():
    assert S3Plugin(profile={}).run_cli(["frobnicate"]) == 2


def test_run_cli_unknown_profile_key_is_config_error(capsys):
    rc = S3Plugin(profile={"bogus_key": 1}).run_cli(["ls", "s3://b"])
    assert rc == 3
    assert "bogus_key" in capsys.readouterr().err


def test_skills_dir_is_class_reachable_and_exists():
    skills = Path(S3Plugin.skills_dir())
    assert (skills / "s3" / "SKILL.md").is_file()
    assert (skills / "s3-setup" / "SKILL.md").is_file()


def test_system_prompt_unconfigured_points_to_setup_skill():
    text = S3Plugin.system_prompt({})
...
def test_config_schema_lists_aws_and_plugin_fields():
    ...
    assert {"bucket", "kms_key_id"} <= names  # plugin extras


def test_validate_profile_shape_checks():
    # ${ENV_VAR} placeholders are opaque and never rejected
    assert S3Plugin.validate_profile({"secret_access_key": "${AWS_SECRET}"}) == []
    # every declared config_schema field is an accepted key
    assert S3Plugin.validate_profile(
        {f["name"]: "${X}" for f in S3Plugin.config_schema()}
    ) == []
    # unknown keys are reported
    errs = S3Plugin.validate_profile({"bogus_key": 1})
    assert errs and "bogus_key" in errs[0]
    # a malformed endpoint URL is caught, a placeholder is not
    assert S3Plugin.validate_profile({"endpoint_url": "ftp://x"})
    assert S3Plugin.validate_profile({"endpoint_url": "${EP}"}) == []
...
def test_cli_permissions_shape():
    perms = S3Plugin.cli_permissions()
...
def test_cli_permissions_patterns_reference_real_commands():
    ...
    for rules in S3Plugin.cli_permissions().values():
        for patterns in rules.values():
            for pattern in patterns:
...
def test_cli_permissions_dangerous_never_auto_run():
    ...
    for profile, rules in perms.items():
        ...
        assert not any(_matches("rm", h) for h in allow_heads), f"rm must never auto-run ({profile})"
...
def test_cli_permissions_routine_promoted():
    ...
    for command in ("ls", "cat", "select", "buckets list"):
        assert allowed("normal", command) and allowed("auto", command)
    for command in ("cp", "sync", "mv"):
...
def test_package_never_imports_datus():
    result = subprocess.run(
        [
            sys.executable,
            "-c",
            "import sys; import datus_s3_plugin.plugin; import datus_s3_plugin.cli; "
