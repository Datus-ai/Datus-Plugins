...
def test_config_schema_lists_aws_and_plugin_fields():
    schema = GluePlugin.config_schema()
    ...
    assert "catalog_id" in names  # plugin extra


def test_validate_profile_shape_checks():
    # ${ENV_VAR} placeholders are opaque and never rejected
    assert GluePlugin.validate_profile({"secret_access_key": "${AWS_SECRET}"}) == []
    # every declared config_schema field is an accepted key
    assert GluePlugin.validate_profile(
        {f["name"]: "${X}" for f in GluePlugin.config_schema()}
    ) == []
    # unknown keys are reported
    errs = GluePlugin.validate_profile({"bogus_key": 1})
    assert errs and "bogus_key" in errs[0]
    # a malformed endpoint URL is caught, a placeholder is not
    assert GluePlugin.validate_profile({"endpoint_url": "ftp://x"})
    assert GluePlugin.validate_profile({"endpoint_url": "${EP}"}) == []
