...
def test_config_schema_lists_aws_and_plugin_fields():
    schema = EcsPlugin.config_schema()
    ...
    assert {"cluster", "log_group"} <= names  # plugin extras


def test_validate_profile_shape_checks():
    # ${ENV_VAR} placeholders are opaque and never rejected
    assert EcsPlugin.validate_profile({"secret_access_key": "${AWS_SECRET}"}) == []
    # every declared config_schema field is an accepted key
    assert EcsPlugin.validate_profile(
        {f["name"]: "${X}" for f in EcsPlugin.config_schema()}
    ) == []
    # unknown keys are reported
    errs = EcsPlugin.validate_profile({"bogus_key": 1})
    assert errs and "bogus_key" in errs[0]
    # a malformed endpoint URL is caught, a placeholder is not
    assert EcsPlugin.validate_profile({"endpoint_url": "ftp://x"})
    assert EcsPlugin.validate_profile({"endpoint_url": "${EP}"}) == []
