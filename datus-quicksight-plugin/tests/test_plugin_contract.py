...
def test_config_schema_lists_aws_and_plugin_fields():
    schema = QuickSightPlugin.config_schema()
    ...
    assert {"aws_account_id", "namespace", "identity_region"} <= names  # plugin extras
    # namespace ships a default in the form
    assert any(f["name"] == "namespace" and f.get("default") == "default" for f in schema)


def test_validate_profile_shape_checks():
    # ${ENV_VAR} placeholders are opaque and never rejected
    assert QuickSightPlugin.validate_profile({"secret_access_key": "${AWS_SECRET}"}) == []
    # every declared config_schema field is an accepted key
    assert QuickSightPlugin.validate_profile(
        {f["name"]: "${X}" for f in QuickSightPlugin.config_schema()}
    ) == []
    # unknown keys are reported
    errs = QuickSightPlugin.validate_profile({"bogus_key": 1})
    assert errs and "bogus_key" in errs[0]
    # a malformed endpoint URL is caught, a placeholder is not
    assert QuickSightPlugin.validate_profile({"endpoint_url": "ftp://x"})
    assert QuickSightPlugin.validate_profile({"endpoint_url": "${EP}"}) == []
