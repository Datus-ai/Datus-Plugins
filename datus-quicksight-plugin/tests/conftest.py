...
from datus_quicksight_plugin.config import Settings
...
@pytest.fixture
def clients():
    # "quicksight" = regional client; "quicksight-identity" = identity-region client
    return {"quicksight": FakeBotoClient(), "quicksight-identity": FakeBotoClient()}


@pytest.fixture
def run_cli(clients):
    def _run(argv: List[str], profile: Optional[Dict[str, Any]] = None) -> int:
        ctx = AwsContext(Settings.from_profile(profile or {"region": "us-east-1", "aws_account_id": "123456789012"}))
