"""The Datus plugin contract for datus-iam-plugin (pure read-only)."""
...
from datus_iam_plugin.plugin import IamPlugin

PKG_DIR = Path(__file__).resolve().parents[1] / "datus_iam_plugin"
GROUPS = ("whoami", "roles", "users", "policies", "simulate")


def test_constructor_accepts_profile_keyword():
    plugin = IamPlugin(profile={"name": "prod", "region": "us-east-1"})
    assert plugin.profile["name"] == "prod"
    assert IamPlugin().profile == {}


def test_run_cli_help_returns_zero(capsys):
    rc = IamPlugin(profile={}).run_cli(["--help"])
    assert rc == 0
...
def test_run_cli_unknown_command_returns_usage_error():
    assert IamPlugin(profile={}).run_cli(["frobnicate"]) == 2


def test_run_cli_unknown_profile_key_is_config_error(capsys):
    rc = IamPlugin(profile={"bogus_key": 1}).run_cli(["roles", "list"])
    assert rc == 3
    assert "bogus_key" in capsys.readouterr().err


def test_skills_dir_is_class_reachable_and_exists():
    skills = Path(IamPlugin.skills_dir())
    assert (skills / "iam" / "SKILL.md").is_file()
    assert (skills / "iam-setup" / "SKILL.md").is_file()


def test_system_prompt_unconfigured_points_to_setup_skill():
    text = IamPlugin.system_prompt({})
    assert "not configured" in text
    assert "iam-setup" in text
...
def test_system_prompt_lists_environments_without_secrets():
    ...
    text = IamPlugin.system_prompt(profiles)
    assert "## IAM" in text
...
def _cli_command_paths() -> list:
    from datus_iam_plugin.cli import build_parser
...
def test_cli_permissions_shape():
    perms = IamPlugin.cli_permissions()
...
def test_cli_permissions_patterns_reference_real_commands():
    ...
    for rules in IamPlugin.cli_permissions().values():
        for patterns in rules.values():
            for pattern in patterns:
...
def test_everything_is_read_only_allowed_in_both_profiles():
    perms = IamPlugin.cli_permissions()
    ...
    for command in _cli_command_paths():
        assert allowed("normal", command) and allowed("auto", command), command
    # a truly read-only plugin has nothing to confirm
    assert perms["normal"]["ask"] == [] and perms["auto"]["ask"] == []
...
def test_package_never_imports_datus():
    result = subprocess.run(
        [
            sys.executable,
            "-c",
            "import sys; import datus_iam_plugin.plugin; import datus_iam_plugin.cli; "
