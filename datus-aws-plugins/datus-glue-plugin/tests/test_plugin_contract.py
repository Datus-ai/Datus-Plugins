"""The declarative Datus plugin contract (datus-plugin.yml) for datus-glue-plugin."""
...
def test_manifest_declares_the_module_contract():
    ...
    assert manifest["cli"] == "datus_glue_plugin.cli:main"
...
def test_entry_point_is_a_module_ref():
    ...
    assert entry_points == {PLUGIN_NAME: "datus_glue_plugin"}
...
def test_cli_unknown_profile_key_is_config_error(capsys):
    main = resolve_cli(load_manifest())
    rc = main(["catalog", "databases"], {"bogus_key": 1})
    assert rc == 3
...
def test_config_schema_marks_secret_fields():
    ...
    for referenced in ("region", "profile", "role_arn", "access_key_id", "catalog_id"):
...
def test_package_never_imports_datus():
    result = subprocess.run(
        [
            sys.executable,
            "-c",
            "import sys; import datus_glue_plugin.cli; "
