"""The declarative Datus plugin contract (datus-plugin.yml) for datus-ecs-plugin."""
...
def test_manifest_declares_the_module_contract():
    ...
    assert manifest["cli"] == "datus_ecs_plugin.cli:main"
...
def test_entry_point_is_a_module_ref():
    ...
    assert entry_points == {PLUGIN_NAME: "datus_ecs_plugin"}
...
def test_config_schema_marks_secret_fields():
    ...
    for referenced in ("region", "profile", "role_arn", "access_key_id", "cluster"):
...
def test_package_never_imports_datus():
    result = subprocess.run(
        [
            sys.executable,
            "-c",
            "import sys; import datus_ecs_plugin.cli; "
