"""The Datus plugin contract for datus-emr-serverless-plugin."""
...
from datus_emr_serverless_plugin.plugin import EmrServerlessPlugin

PKG_DIR = Path(__file__).resolve().parents[1] / "datus_emr_serverless_plugin"
GROUPS = ("applications", "jobs")


def test_constructor_accepts_profile_keyword():
    plugin = EmrServerlessPlugin(profile={"name": "prod", "region": "us-east-1"})
    assert plugin.profile["name"] == "prod"
    assert EmrServerlessPlugin().profile == {}


def test_run_cli_help_returns_zero(capsys):
    rc = EmrServerlessPlugin(profile={}).run_cli(["--help"])
    assert rc == 0
...
def test_run_cli_unknown_command_returns_usage_error():
    assert EmrServerlessPlugin(profile={}).run_cli(["frobnicate"]) == 2


def test_run_cli_unknown_profile_key_is_config_error(capsys):
    rc = EmrServerlessPlugin(profile={"bogus_key": 1}).run_cli(["applications", "list"])
    assert rc == 3
    assert "bogus_key" in capsys.readouterr().err


def test_skills_dir_is_class_reachable_and_exists():
    skills = Path(EmrServerlessPlugin.skills_dir())
    assert (skills / "emr-serverless" / "SKILL.md").is_file()
    assert (skills / "emr-serverless-setup" / "SKILL.md").is_file()


def test_system_prompt_unconfigured_points_to_setup_skill():
    text = EmrServerlessPlugin.system_prompt({})
    assert "not configured" in text
    assert "emr-serverless-setup" in text
...
def test_system_prompt_lists_environments_without_secrets():
    ...
    text = EmrServerlessPlugin.system_prompt(profiles)
    assert "## EMR Serverless" in text
    assert "us-east-1" in text and "app-123" in text
    assert "s3cr3t-key-value" not in text and "AKIAEXAMPLE" not in text


def test_config_schema_lists_aws_and_plugin_fields():
    schema = EmrServerlessPlugin.config_schema()
    ...
    assert {"application_id", "execution_role_arn"} <= names  # plugin extras


def test_validate_profile_shape_checks():
    # ${ENV_VAR} placeholders are opaque and never rejected
    assert EmrServerlessPlugin.validate_profile({"secret_access_key": "${AWS_SECRET}"}) == []
    # every declared config_schema field is an accepted key
    assert EmrServerlessPlugin.validate_profile(
        {f["name"]: "${X}" for f in EmrServerlessPlugin.config_schema()}
    ) == []
    # unknown keys are reported
    errs = EmrServerlessPlugin.validate_profile({"bogus_key": 1})
    assert errs and "bogus_key" in errs[0]
    # a malformed endpoint URL is caught, a placeholder is not
    assert EmrServerlessPlugin.validate_profile({"endpoint_url": "ftp://x"})
    assert EmrServerlessPlugin.validate_profile({"endpoint_url": "${EP}"}) == []
...
def test_cli_permissions_shape():
    perms = EmrServerlessPlugin.cli_permissions()
...
def test_cli_permissions_patterns_reference_real_commands():
    ...
    for rules in EmrServerlessPlugin.cli_permissions().values():
        for patterns in rules.values():
            for pattern in patterns:
...
def test_cli_permissions_dangerous_never_auto_run():
    ...
    for profile, rules in perms.items():
        ...
        assert not any(_matches("jobs run", h) for h in allow_heads), f"jobs run must never auto-run ({profile})"
...
def test_cli_permissions_routine_promoted():
    ...
    for command in ("applications list", "jobs dashboard"):
        assert allowed("normal", command) and allowed("auto", command)
    for command in ("applications start", "jobs cancel"):
...
def test_package_never_imports_datus():
    result = subprocess.run(
        [
            sys.executable,
            "-c",
            "import sys; import datus_emr_serverless_plugin.plugin; import datus_emr_serverless_plugin.cli; "
